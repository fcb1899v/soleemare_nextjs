...
const InquaryBody: NextPage = () => { 
  ...
  const formLabel = ["姓", "名", "メールアドレス", "お電話番号", "お問い合わせ内容"]
  ...
  return (
    <div id="inquary" className={styles.container}>
      <h2><span>Richiesta</span><br/>お問い合わせフォーム</h2>
      <form action={formUrl} method="POST" target="hidden_iframe" onSubmit={handleSubmit}>
        <div className={styles.name_container}>
          <TextField className={styles.text_field} type="text" required
            name={formNumber[0]} placeholder={formPlaceholder[0]}
            label={<>
              <AccountIcon fontSize="small" className={styles.label_icon}/>
              {formLabel[0]}
            </>} 
            value={familyName} onChange={handleFamilyNameChange}
          />
          <TextField className={styles.text_field} type="text" required 
            placeholder={formPlaceholder[1]} name={formNumber[1]} 
            label={<>
              <AccountIcon fontSize="small" className={styles.label_icon}/>
              {formLabel[1]}
            </>} 
        ...
        <TextField className={styles.text_field} type="text" required 
            name={formNumber[2]} placeholder={formPlaceholder[2]}
            label={<>
              <MailIcon fontSize="small" className={styles.label_icon}/>
              {formLabel[2]}
            </>} 
            value={email} onChange={handleEmailChange}
        />
        <TextField className={styles.text_field} type="text" required 
            name={formNumber[3]} placeholder={formPlaceholder[3]}
            label={<>
              <PhoneIcon fontSize="small" className={styles.label_icon}/>
              {formLabel[3]}
            </>} 
            value={phone} onChange={handlePhoneChange}
        />
        <TextField className={styles.text_field} type="text" required multiline rows={10}
            name={formNumber[4]} placeholder={formPlaceholder[4]}
            label={<>
              <Edit fontSize="small" className={styles.label_icon}/>
              {formLabel[4]}
            </>} 
